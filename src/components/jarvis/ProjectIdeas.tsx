...
import { Lightbulb, Code, ChevronDown, ChevronUp, Copy, Check, Sparkles, X, FolderTree, FileText, Folder, Download } from "lucide-react";
...
export function ProjectIdeas({ ideas, onGenerateIdeas, isGenerating }: ProjectIdeasProps) {
  ...
  const exportAsReadme = (idea: ProjectIdea) => {
    const fileTree = idea.fileStructure?.length
      ? idea.fileStructure.map(p => `  ${p}`).join("\n")
      : "  (no structure provided)";

    const codeBlock = idea.codeSnippet
      ? `## Starter Code\n\n**File:** \`${idea.codeFile || "index.ts"}\`\n\n\`\`\`\n${idea.codeSnippet}\n\`\`\``
      : "";

    const md = `# ${idea.title}

> **Difficulty:** ${idea.difficulty} | **Tags:** ${idea.tags.join(", ")}
...
`;
    ...
    a.href = url;
    a.download = `${idea.title.toLowerCase().replace(/\s+/g, "-")}-README.md`;
    a.click();
